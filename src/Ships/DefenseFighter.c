...
/*        if (defensestruct->bullet)
        ...
        {   //laser cannon isn't cooling down, so continue tracking
            ...
            if((universe.univUpdateCounter % (defensefighterstatics->DamageRate * UNIVERSE_UPDATE_RATE_FACTOR)) == 0)
            {
                // Time to do damage...
                //Do damage to bullet
            ...
            if(defensestruct->laser != NULL)
            {
                ...
                if((universe.univUpdateCounter % (defensefighterstatics->RangeCheckRate * UNIVERSE_UPDATE_RATE_FACTOR)) == 0)
                {
