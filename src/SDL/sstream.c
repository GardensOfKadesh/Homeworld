...
// no thread based update function for emscripten
int isoundstreamupdate_manual(void *dummy)
{
    ...
    // we don't want to run out of sound samples since we do everything in one thread in emscripten
    for (int rounds = 0; rounds < 1; rounds++) {
    if (soundinited && (streamer.status >= SOUND_STOPPED))
	{
		if (streamer.status == SOUND_STOPPING)
		{
