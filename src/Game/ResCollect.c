...
bool CollectResources(Ship *ship,Resource *resource)
{
    ...
    if ((universe.univUpdateCounter % (shipstatic->harvestRate * UNIVERSE_UPDATE_RATE_FACTOR)) == 0)
    {
...
letsdock:
    ...
    else
    {
        ...
        if ((universe.univUpdateCounter % (CHECK_HARV_OUTOFWORLD_RESOURCE_RATE * UNIVERSE_UPDATE_RATE_FACTOR)) == (resource->resourceID.resourceNumber & CHECK_HARV_OUTOFWORLD_RESOURCE_RATE))
        {
