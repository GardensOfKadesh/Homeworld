...
//fix later and optimize by passing in a rotate flag
    ...
    for (i=0;i<numShips;i++)
    {
        ...
        if (attackvars->multipleAttackTargets)
        {
            ...
            if ((universe.univUpdateCounter % (MULTIPLETARGETS_RETARGET_RATE * UNIVERSE_UPDATE_RATE_FACTOR)) == (ship->shipID.shipNumber & MULTIPLETARGETS_RETARGET_RATE))
            {
...
bool processAttackToDo(CommandToDo *attacktodo)
{
    ...
    for (i=0;i<numShips;i++)
    {
        ...
        else
        {
            ...
            if (target != NULL)
            {
                if ((universe.univUpdateCounter % (checkbogeyrate * UNIVERSE_UPDATE_RATE_FACTOR)) == (CHECKBOGEY_FRAME & checkbogeyrate))
                {
                    ...
                    else
                    {
                        ...
                        if ((ship->tacticstype == Evasive) && ((universe.univUpdateCounter % (FLIGHTMAN_TOKEN_EVASIVEPURE_CHECKRATE * UNIVERSE_UPDATE_RATE_FACTOR)) == FLIGHTMAN_TOKEN_EVASIVEPURE_CHECKFRAME))
                        {
    ...
    for (i=0;i<selection->numShips;i++)
    {
        ...
        if(i >= 1)
        {
            ...
            if(attacktodo->formation.formationtype == SPHERE_FORMATION)
            {
                ...
                if ((universe.univUpdateCounter % (SPHERE_CHECK_TARGET_MASK * UNIVERSE_UPDATE_RATE_FACTOR)) == SPHERE_CHECK_TARGET_FRAME)
                {
                        ...
                        {   //evasive tactics, so attack as per olden times
                            ...
                            if ((command->formation.doneInitialAttack) || (command->attack->numTargets == 0) || (TargetWithinAttackRange(ship,attacktarget)))
                            {
                                ...
                                else
                                {
                                    if(command->ordertype.attributes & COMMAND_MASK_ATTACKING_AND_MOVING)
                                    {
                                        if((universe.univUpdateCounter % (CHECK_ATTACKMOVE_TO_MOVE_RATE * UNIVERSE_UPDATE_RATE_FACTOR))==CHECK_ATTACKMOVE_TO_MOVE_FRAME)
